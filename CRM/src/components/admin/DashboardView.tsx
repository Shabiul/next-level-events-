  ...
  useEffect(() => {
    authFetch(API)
      .then(async (r) => {
        ...
        if (!r.ok) throw new Error(body?.message || body?.error || 'Failed to load dashboard data');
        return body;
      })
      .then(d => { setData(d); setLoading(false); })
      .catch((err) => { setError(err instanceof Error ? err.message : 'Failed to load dashboard data'); setLoading(false); });
  }, []);
