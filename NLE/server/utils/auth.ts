...
/**
 * "staff" is a restricted admin account: it can sign into the CRM like an
 * admin, but only reach the sections listed in `permissions` (matching the
 * CRM's AdminView keys -- 'products', 'orders', 'payments', ...). `admin`
 * implicitly has every permission. Permissions are baked into the token
 * itself (not looked up per request) so route guards stay a single JWT
 * verify, same cost as before -- staff scopes rarely change, and the
 * console-side CRUD that edits them also re-issues a fresh token isn't
 * needed since the staff member just needs to re-login to pick up a change,
 * same as a role change already required.
 */
...
export function verifyAuthToken(token: string): TokenPayload | null {
  try {
    ...
    if (decoded && typeof decoded === "object" && "id" in decoded) {
      ...
      const role = d.role === "admin" ? "admin" : d.role === "staff" ? "staff" : "user";
      return {
        id: String(d.id),
        role,
        permissions: Array.isArray(d.permissions) ? d.permissions.map(String) : undefined,
      };
    }
    return null;
  } catch {
...
/**
 * Rejects the request unless the caller is an admin, or a staff account
 * whose token carries the given scope. Use this in place of `requireAdmin`
 * on any admin-console route that should be narrowable to specific staff.
 */
export function requirePermission(scope: string) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const allowed = payload.role === "admin" || (payload.role === "staff" && payload.permissions?.includes(scope));
    if (!allowed) {
