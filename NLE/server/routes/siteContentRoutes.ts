...
function requireContentScope(req: Request, res: Response, next: NextFunction) {
  ...
  return requirePermission(scope)(req, res, next);
}
...
router.get("/:key", async (req: Request, res: Response) => {
  try {
    ...
    const doc = await SiteContentRepository.getByKey(key);
    if (!doc) {
    ...
    const defKey = key as keyof typeof DEFAULTS;
    const def = DEFAULTS[defKey];
    if (def) return res.json({ key, ...def });
    return res.status(500).json({ msg: "Server error" });
  }
});

router.put("/:key", requireContentScope, async (req: Request, res: Response) => {
  try {
    const key = String(req.params.key);
