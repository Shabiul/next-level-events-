...
export const ProductDetailView: React.FC<Props> = ({ product, onBack, onBook }) => {
  ...
  // Single-open accordion: picking a new tab closes whichever one was open first.
  ...
  const isAlreadyInCart = useMemo(() => items.some((i) => i._id === product._id), [items, product._id]);

  const handleRemoveAddon = (idOrName: string) => {
    setGlobalSelections((prev) => ({
      addons: prev.addons.filter((a) => String(a.id || a.name) !== String(idOrName)),
      activities: prev.activities.filter((a) => String(a.id || a.name) !== String(idOrName)),
    }));
  };
  ...
  const handleAddToCart = () => {
    ...
    if (bookingSelections.length > 0) {
      toast.success(`"${product.name}" with ${bookingSelections.length} add-on${bookingSelections.length > 1 ? 's' : ''} added to cart!`);
    } else {
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-7xl mx-auto w-full flex flex-col gap-9">
          ...
          >
            ...
            <div className="mt-6 flex flex-wrap items-baseline gap-4 border-t border-[#381932]/60 pt-6 dark:border-[#381932]/60">
              <div className="flex flex-col">
                ...
                {bookingSelections.length > 0 && (
                  <p className="text-xs font-semibold text-[#381932]/80 dark:text-[#FFF3E6]/80 mt-1">
                    Base Package: ₹{product.price.toLocaleString('en-IN')} + {bookingSelections.length} Add-on{bookingSelections.length > 1 ? 's' : ''} (+₹{(totalPrice - product.price).toLocaleString('en-IN')})
                  </p>
                )}
              </div>
            ...
            {/* Selected Add-ons summary chips */}
            {bookingSelections.length > 0 && (
              <div className="mt-4 p-3.5 rounded-2xl bg-[#A78A9F]/10 border border-[#381932]/20 dark:border-[#FFF3E6]/20">
                <div className="flex items-center justify-between gap-2 mb-2">
                  <span className="text-xs font-bold uppercase tracking-wider text-[#381932] dark:text-[#FFF3E6] flex items-center gap-1.5">
                    <CheckCircle2 size={14} className="text-green-600 dark:text-green-400" />
                    Selected Add-ons ({bookingSelections.length})
                  </span>
                  <span className="text-xs font-bold text-[#381932] dark:text-[#FFF3E6]">
                    +₹{(totalPrice - product.price).toLocaleString('en-IN')}
                  </span>
                </div>
                <div className="flex flex-wrap gap-2">
                  {bookingSelections.map((sel) => (
                    <span
                      key={sel.id || sel.name}
                      className="inline-flex items-center gap-1.5 bg-[#FFF3E6] dark:bg-[#381932] border border-[#381932]/30 dark:border-[#FFF3E6]/30 px-2.5 py-1 rounded-full text-xs font-semibold text-[#381932] dark:text-[#FFF3E6] shadow-2xs"
                    >
                      <span>{sel.name}</span>
                      <span className="text-[#381932]/70 dark:text-[#FFF3E6]/70">₹{(sel.price || 0).toLocaleString('en-IN')}</span>
                      <button
                        type="button"
                        onClick={() => handleRemoveAddon(sel.id || sel.name)}
            ...
            <div className="mt-7 grid grid-cols-1 sm:grid-cols-3 gap-3.5">
              ...
              >
                ...
                <span>{isAlreadyInCart && bookingSelections.length > 0 ? 'Update Cart with Add-ons' : (isAlreadyInCart ? 'In Cart (Add Again)' : 'Add to Cart')}</span>
              </motion.button>
          ...
          <div className="w-full flex flex-col gap-10">
            ...
            <div className="rounded-2xl border border-[#381932]/30 bg-[#FFF3E6]/95 px-3 py-3 sm:px-5 dark:bg-[#381932]/95 dark:border-[#381932] shadow-card backdrop-blur-md">
              <div className="flex flex-wrap items-center gap-x-2 gap-y-2 sm:gap-x-4">
                {POLICY_TABS.map((tab) => {
                  const isOpen = openPolicy === tab.key;
                  return (
                    <button
                      key={tab.key}
                      type="button"
            ...
            <AnimatePresence initial={false}>
              {POLICY_TABS.filter((tab) => tab.key === openPolicy).map((tab) => {
            ...
            >
              <AddonsModule
                onSelectionChange={handleGlobalSelectionChange}
                selectedAddonIds={globalSelections.addons.map((a) => a.id || a.name)}
                selectedActivityIds={globalSelections.activities.map((a) => a.id || a.name)}
                themeCategory={product.categoryName}
              />
            </motion.div>
        ...
        {/* Fullscreen Photo Modal -- portaled to <body>. This page wraps its
            content in a transformed motion.div (3D parallax tilt) further
            up the tree; a transformed ancestor becomes the containing block
            for every `fixed` descendant (CSS spec), so without the portal
            this modal was positioning/clipping against that local wrapper
            instead of the viewport -- which is why it rendered stuck behind
            the site header with its close button unreachable. */}
        {createPortal(
          <AnimatePresence>
            {fullScreenModalOpen && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 z-[999999] flex flex-col bg-[#381932]/95 backdrop-blur-md p-4"
        ...
        {/* Floating Celebration Upgrades Bar (Desktop / Tablet) */}
        <AnimatePresence>
          {bookingSelections.length > 0 && (
            <motion.div
              ...
              className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-4xl hidden sm:block"
            >
              <div className="rounded-2xl border border-[#381932]/40 bg-[#FFF3E6]/95 dark:bg-[#381932]/95 backdrop-blur-xl p-4 shadow-[0_20px_50px_rgba(56,25,50,0.25)] flex items-center justify-between gap-4">
                <div className="flex flex-col min-w-0">
                  <div className="flex items-center gap-2.5">
                    <span className="font-serif text-2xl font-extrabold text-[#381932] dark:text-[#FFF3E6]">
                      ₹{totalPrice.toLocaleString('en-IN')}
                    </span>
                    <span className="text-xs font-bold text-emerald-800 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-950/70 px-2.5 py-0.5 rounded-full border border-emerald-300 dark:border-emerald-800">
                      ✓ {bookingSelections.length} Add-on{bookingSelections.length > 1 ? 's' : ''} Attached
                    </span>
                  </div>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-xs text-[#381932]/80 dark:text-[#FFF3E6]/80 font-medium truncate max-w-md">
                      {product.name} (Base ₹{product.price.toLocaleString('en-IN')} + ₹{(totalPrice - product.price).toLocaleString('en-IN')} add-ons)
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5 flex-shrink-0">
                  <button
                    type="button"
                    onClick={handleAddToCart}
                    className="inline-flex items-center gap-2 rounded-xl border border-[#381932] bg-[#FFF3E6] hover:bg-[#A78A9F]/20 text-[#381932] dark:border-[#FFF3E6] dark:bg-[#381932] dark:text-[#FFF3E6] dark:hover:bg-[#FFF3E6]/10 px-4 py-2.5 text-xs sm:text-sm font-bold tracking-wide transition-all cursor-pointer shadow-sm"
                  >
                    <ShoppingCart size={16} />
                    <span>{isAlreadyInCart ? 'Update Cart with Add-ons' : 'Add to Cart'}</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => handleBookNow('razorpay')}
                    className="inline-flex items-center gap-2 rounded-xl bg-[#381932] hover:opacity-90 text-[#FFF3E6] dark:bg-[#FFF3E6] dark:text-[#381932] px-5 py-2.5 text-xs sm:text-sm font-bold tracking-wide shadow-md transition-all cursor-pointer"
                  >
        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-[#381932]/30 bg-[#FFF3E6]/95 backdrop-blur-md p-3.5 shadow-modal sm:hidden flex items-center justify-between gap-2.5 dark:bg-[#381932]/95 dark:border-[#381932]">
          <div className="min-w-0 flex-1">
            <div className="text-[10px] uppercase font-bold text-[#381932] dark:text-[#FFF3E6]/70">
              {bookingSelections.length > 0 ? `Total (${bookingSelections.length} Add-on${bookingSelections.length > 1 ? 's' : ''})` : 'Total'}
            </div>
            <div className="text-base font-bold text-[#381932] dark:text-[#FFF3E6] truncate">₹{totalPrice.toLocaleString('en-IN')}</div>
          </div>
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <button
              type="button"
              onClick={handleAddToCart}
              className="flex h-10 px-3 items-center justify-center gap-1.5 rounded-xl border border-[#381932] bg-[#FFF3E6] text-[#381932] text-xs font-bold dark:border-[#FFF3E6] dark:bg-[#381932] dark:text-[#FFF3E6] cursor-pointer"
            ...
            <button
              type="button"
              onClick={() => handleBookNow('whatsapp')}
              className="flex h-10 w-10 items-center justify-center rounded-xl border border-[#381932] bg-[#FFF3E6] text-[#381932] dark:bg-[#381932]/40 dark:text-[#FFF3E6]"
              aria-label="WhatsApp"
            >
              {WA_SVG}
            </button>
