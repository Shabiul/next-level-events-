    ...
    >
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto max-w-2xl text-center mb-8 sm:mb-10">
          ...
          <h2 className="font-serif text-3xl font-semibold tracking-tight text-[#34203C] dark:text-[#FAF8F5] sm:text-4xl lg:text-[42px] leading-[1.15]">
            {title}
          </h2>
          <p className="mt-2 text-xs sm:text-sm md:text-[15px] font-light leading-relaxed text-[#725D75] dark:text-[#C8B5C3]">
            {subtitle}
          </p>
        </div>
        ...
        <div className="flex items-center justify-center mb-6 sm:mb-8">
          <div className="flex flex-wrap items-center justify-center gap-1.5 p-1 rounded-full bg-[#F5EFE6] dark:bg-[#25172C] border border-[#DDD5C7] dark:border-[#483250] shadow-xs">
            {TABS.map((tab) => {
                ...
                <button
                  ...
                  className={`relative z-10 rounded-full px-3.5 py-1.5 text-xs sm:text-[13px] font-medium transition-colors duration-200 cursor-pointer ${
                    isActive
                      ...
                      : 'text-[#725D75] hover:text-[#34203C] dark:text-[#C8B5C3] dark:hover:text-[#FAF8F5] hover:bg-[#FAF8F5]/60'
                  }`}
                >
                  {isActive && (
                    <motion.div
                      layoutId="activeFaqTab"
                      className="absolute inset-0 z-[-1] rounded-full bg-[#34203C] dark:bg-[#483250] shadow-sm"
        ...
        <div className="mx-auto max-w-3xl rounded-2xl sm:rounded-3xl border border-[#DDD5C7] dark:border-[#483250] bg-white dark:bg-[#25172C] shadow-sm overflow-hidden divide-y divide-[#DDD5C7]/60 dark:divide-[#483250]/60">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="divide-y divide-[#DDD5C7]/60 dark:divide-[#483250]/60"
            >
              {filteredFAQs.map((item) => {
                ...
                return (
                  <div key={item.id} className="transition-colors hover:bg-[#FAF8F5]/50 dark:hover:bg-[#34203C]/20">
                    ...
                    >
                      <span className="text-xs sm:text-sm md:text-[15px] font-medium text-[#34203C] dark:text-[#FAF8F5] tracking-tight pr-2">
                        {item.question}
                      </span>
                      <motion.div
                        animate={{ rotate: isOpen ? 180 : 0 }}
                        transition={{ duration: 0.22, ease: 'easeInOut' }}
                        className="flex h-6 w-6 sm:h-7 sm:w-7 flex-shrink-0 items-center justify-center rounded-full text-[#725D75] dark:text-[#C8B5C3]"
                      >
                      ...
                      {isOpen && (
                        ...
                        >
                          <div className="px-4.5 sm:px-6 pb-5 pt-0">
                            <p className="text-xs sm:text-[13px] font-light leading-relaxed text-[#725D75] dark:text-[#C8B5C3]">
                              {item.answer}
                            </p>
                          </div>
                        </motion.div>
        ...
        <div className="mt-6 sm:mt-8 text-center text-xs sm:text-[13px] text-[#725D75] dark:text-[#C8B5C3]">
          Have a question not listed here?{' '}
          <a
            href="https://wa.me/917022058460"
            target="_blank"
            rel="noreferrer"
            className="font-semibold text-[#34203C] dark:text-[#FAF8F5] underline underline-offset-4 hover:text-[#A78A9F] transition-colors"
          >
